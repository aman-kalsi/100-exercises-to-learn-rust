...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError { explanation }) => {
            panic!("{explanation}");
        },
        Err(TicketNewError::DescriptionError { explanation }) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { explanation: "Title cannot be empty".into() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { explanation: "Title cannot be longer than 50 bytes".into() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { explanation: "Description cannot be empty".into() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { explanation: "Description cannot be longer than 500 bytes".into() });
        }
